...
### 画像カラーピッカー
- PNG・JPEG・WebP画像のファイル選択、ドラッグ＆ドロップ読み込み
- クリック・タップした位置のHEX、RGB、RGBA、HSLを表示・コピー
- ピクセル拡大ルーペ、透明色プレビュー、最大8件の取得履歴に対応
- RGB（0～255）・Alpha（0.0～1.0）スライダー、数値入力、標準カラーピッカーによる色の微調整
- カラーパレット・グラデーション・透明度のサンプル画像を収録
- 画像はサーバーへ送信せず、ブラウザ内だけで処理
